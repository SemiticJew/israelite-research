...
(function () {
  ...
  Promise.allSettled([lexPromise, chapPromise])
    .then(results => {
      const lexResult = results[0];
      const chapResult = results[1];
      if (chapResult.status === 'fulfilled') {
        try { renderChapter(chapResult.value); } catch (e) { console.error('Render error:', e); }
      } else {
        throw chapResult.reason || new Error('Chapter fetch failed');
      }
    })
    .catch(err => {
      if (versesEl) {
