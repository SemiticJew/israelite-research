...
<script>
const book='${slug}',total=${total},base=\`/israelite-research/newtestament/chapter.html?book=\${book}&ch=\`;
const g=document.getElementById('chapGrid');
for(let i=1;i<=total;i++){const a=document.createElement('a');a.href=base+i;a.className='ch';a.textContent=i;g.appendChild(a);}
