(function(){
  ...
  function chapterHref(c,b,ch){return`/${c}/chapter.html?book=${b}&ch=${ch}`;}
  const canon=canonFromPath(),book=bookFromPath();
  async function getTotal(){try{const r=await fetch(`/data/${canon}/books.json`);if(r.ok){const j=await r.json();return Number(j[book])||150;}}catch{}return 150;}
