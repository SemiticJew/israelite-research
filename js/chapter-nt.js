...
// Also wires breadcrumbs and prev/next chapter buttons.
...
(function () {
  ...
  function renderChapter(data) {
    ...
    data.verses.forEach(v => {
      ...
      // Style (match Tanakh): background #054A91, white text
  ...
  function buildCommentary(v, { book, chapter }) {
    const key = `commentary:${book}:${chapter}:${v.num}`; // keep same scheme as Tanakh
...
// Prev/Next chapter controls (hide when not applicable)
(function(){
  const BASE = '/israelite-research';
  const qs = new URLSearchParams(location.search);
  const book = qs.get('book') || 'Revelation';
  ...
  // Try to hide "next" if this is the last chapter by consulting books.json
  fetch(`${BASE}/data/newtestament/books.json`, { cache: 'no-store' })
    .then(r => r.ok ? r.json() : null)
    .then(json => {
      if (!json || !Array.isArray(json.books)) return;
      const entry = json.books.find(b => (b.name || '').toLowerCase() === book.toLowerCase());
      if (entry && typeof entry.chapters === 'number' && next && chapter >= entry.chapters) {
